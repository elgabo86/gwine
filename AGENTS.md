...
- `patches/*.mypatch` — Patches personnalisés copiés dans wine-tkg-userpatches/
  ...
  - `apply-faketime.py` — Script Python exécuté après le staging wine-tkg et avant `make_requests`. Modifie `server/protocol.def` (ajout `@REQ(set_faketime)`) pour que `make_requests` régénère automatiquement `server_protocol.h` et `request_handlers.h` avec les entrées faketime. Modifie aussi `server/fd.c` (variable `faketime`, handler `DECL_HANDLER(set_faketime)`, ajustement de `current_time`) et `loader/wine.inf.in` (GUID de profil hardware). Injecté dans `wine-tkg-scripts/prepare.sh` par `test-build.sh` avant l'appel à `tools/make_requests`. Portée : gwine uniquement
  - `faketime.mypatch` — Patch de référence (non utilisé directement) contenant les modifications protocol pour le support faketime. Conservé pour documentation et régénération future. Exclu de `wine-tkg-userpatches/` par `test-build.sh` — les modifications sont appliquées par `apply-faketime.py` à la place.
...
- **Séparation cpuid/faketime** : `cpuid_fault_handler.mypatch` (signal_x86_64.c uniquement) est appliqué via le mécanisme standard de wine-tkg. `faketime.mypatch` est exclu des userpatches — le script `apply-faketime.py` est injecté dans `wine-tkg-scripts/prepare.sh` via `sed` avant `tools/make_requests`, ce qui permet à `make_requests` de régénérer les fichiers protocol à partir de `protocol.def` modifié. Cette séparation évite les conflits entre nos patches et les patches staging de wine-tkg.
