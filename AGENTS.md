...
- `patches/*.mypatch` — Patches personnalisés copiés dans wine-tkg-userpatches/
  ...
  - `cpuid_fault_handler.mypatch` — Ajoute la gestion du CPUID faulting dans le handler SIGSEGV de Wine (`dlls/ntdll/unix/signal_x86_64.c`) : interception des instructions CPUID quand le kernel émet un segfault via `ARCH_SET_CPUID`, spoofing des valeurs CPUID (vendor, features), patching de `KUSER_SHARED_DATA`, et bypass syscall via registres XMM. **Comportement AVX** : activé par défaut (`GWINE_NO_AVX` non défini) ; si le programme émet le leaf CPUID `0x336933` (initialisation du spoofing), AVX/XSAVE/AVX2 basculent automatiquement à 0 pour la session. La variable `GWINE_NO_AVX=1` permet de forcer la désactivation manuellement. Ne contient que `signal_x86_64.c` — les modifications protocol (faketime) sont gérées séparément par `apply-faketime.py`. Portée : gwine uniquement
  - `apply-faketime.py` — Script Python exécuté après le staging wine-tkg et avant `make_requests`. Modifie `server/protocol.def` (ajout `@REQ(set_faketime)`) pour que `make_requests` régénère automatiquement `server_protocol.h` et `request_handlers.h` avec les entrées faketime. Modifie aussi `server/fd.c` (variable `faketime`, handler `DECL_HANDLER(set_faketime)`, ajustement de `current_time`) et `loader/wine.inf.in` (GUID de profil hardware). Injecté dans `wine-tkg-scripts/prepare.sh` par `test-build.sh` ET par le workflow CI (`build-gwine.yml`) avant l'appel à `tools/make_requests`. Portée : gwine uniquement
  - `faketime.mypatch` — Patch de référence (non utilisé directement) contenant les modifications protocol pour le support faketime. Conservé pour documentation et régénération future. Exclu de `wine-tkg-userpatches/` par `test-build.sh` ET par le workflow CI — les modifications sont appliquées par `apply-faketime.py` à la place. Ses hunks sur `server_protocol.h`/`request_handlers.h` (fichiers générés) échouent si appliqué en userpatch.
  - `no_exe_executable_bit.mypatch` — Neutralise `is_file_executable` dans `server/file.c` (retourne toujours 0) : le wineserver ne pose plus le bit exécutable Unix sur les fichiers `.exe`/`.com` créés ou renommés par les programmes Windows (installateurs). Sans ce patch, les `.exe` installés dans `~/Windows/Games` ont `+x`, et KIO (KDE) les traite comme des binaires natifs : le double-clic affiche un dialogue « Exécuter » (non fonctionnel) au lieu d'ouvrir avec le handler par défaut (gwine). Neutraliser la fonction couvre les deux sites d'appel (création `server/file.c`, rename `server/fd.c`) sans toucher au `+x` préexistant. L'exécution directe (`./jeu.exe`) est prise en charge côté Gablue par une règle binfmt_misc (`usr/lib/binfmt.d/gablue-windows.conf`, magic MZ → gwine). Portée : gwine uniquement
  - `winebus_coalesce_axis.mypatch` — Corrige la latence accumulée des événements d'axes SDL dans winebus (`dlls/winebus.sys/bus_sdl.c`, `unix_private.h`, `unixlib.c`). Les manettes modernes (DS4/DualSense/Xbox Series) pollent à 250-1000 Hz, mais le backend SDL de winebus ne consommait qu'un seul événement par appel `wait`. Les événements d'axes s'accumulaient dans la file avec un retard médian mesuré à 3457 ms → perçu comme du drift fantôme dans les menus. Le patch introduit : (1) un drain borné de 256 événements SDL via `SDL_PollEvent()` après chaque `SDL_WaitEventTimeout()`, (2) la coalescence des rapports d'axes consécutifs pour le même device (remplacement du buffer par la dernière valeur). Les événements discrets (boutons, D-pad) restent non-coalescibles pour préserver les transitions exactes. Résultat : latence médiane passe de 3457 ms à 1.1 ms. Origine : commit `1451978` de `CachyOS/wine-cachyos` (branche `_winebus`). Portée : gwine uniquement
  - `winebus_udev_drain.mypatch` — Applique le même principe de drain + coalescence au backend evdev/hidraw de winebus (`dlls/winebus.sys/bus_udev.c`). En mode hidraw (`DisableHidraw=0`), le backend udev lit un seul rapport HID par appel `poll()`, laissant les rapports suivants s'accumuler dans le buffer kernel. Le patch : (1) utilise `bus_event_queue_coalesced_input_report` dans `hidraw_device_read_report` et `lnxev_device_read_report` pour éviter la saturation de la file d'attente, (2) ajoute un drain borné de 256 lectures après chaque `read_report()` dans `udev_bus_wait()`. Complémentaire de `winebus_coalesce_axis.mypatch` qui ne couvre que le backend SDL. Portée : gwine uniquement
  - `hid_ioctl_stub.mypatch` — Stubbe silencieusement l'ioctl HID `0xb0197` dans `dlls/hidclass.sys/device.c`. Certains jeux (ex: DragonSword-Awakening) spamment des ioctls HID vendor-specific non supportés en boucle serrée quand un périphérique hidraw est exposé. Chaque appel traverse wineserver → thread winebus → spinlock → log FIXME, saturant le thread et causant une latence massive des entrées manette. Le patch retourne `STATUS_NOT_SUPPORTED` sans log pour cet ioctl, éliminant le spam et permettant au mode hidraw (`DisableHidraw=0`) de fonctionner sans latence. Portée : gwine uniquement
  - `filestandardinfo.mypatch` — Implémente `FileStandardInformation` (classe 5) dans `default_fd_get_file_info` (`server/fd.c`). Le serveur retournait `STATUS_NOT_IMPLEMENTED` pour cette classe, ce qui causait un spam de `FIXME: server_get_file_info Unsupported info class 5` et une latence wineserver. Le patch utilise `fstat()` sur le fd Unix pour remplir un `FILE_STANDARD_INFORMATION` avec `AllocationSize`, `EndOfFile`, `NumberOfLinks`, `DeletePending` et `Directory`. Portée : gwine uniquement
  - `win32u-fill-depth-only-display-mode.mypatch` — Corrige `source_get_full_mode` dans `dlls/win32u/sysparams.c` : les vieux jeux qui changent uniquement la profondeur de couleur (`DM_BITSPERPEL` seul, sans résolution) recevaient un WARN et échouaient. Le patch remplit `dmPelsWidth`/`dmPelsHeight` (et `dmPosition`) depuis les réglages courants. Origine : Soda 11.0-4 (bottlesdevs/wine commit `93e3ed1`, closes Bottles #4297). Portée : gwine uniquement
  - `mod-loader-prefer-native.mypatch` — Ajoute `EXTRADLLFLAGS = -Wb,--prefer-native` sur `version.dll`, `winmm.dll` et `dsound.dll`. Les mods qui déposent ces DLL à côté de l'exe (mod loaders ASI, etc.) sont chargés automatiquement sans override winecfg. Origine : Soda 11.0-4 (bottlesdevs/wine commit `37aa0ba`). Portée : gwine uniquement
  - `ntdll-transparent-huge-pages.mypatch` — Transparent Huge Pages opt-in dans `dlls/ntdll/unix/virtual.c` : si `GWINE_USE_THP=1`, `madvise(MADV_HUGEPAGE)` est appelé sur les allocations virtuelles (`allocate_virtual_memory`), hors `VPROT_WRITEWATCH`/`VPROT_PLACEHOLDER` (le write-watch du GC .NET/mono exige des pages 4 Ko). Hint seulement — le kernel reste décisionnaire selon `/sys/kernel/mm/transparent_hugepage/enabled`. Variable renommée `SODA_USE_THP` → `GWINE_USE_THP`. Origine : Soda 11.0-4 (bottlesdevs/wine commit `69d56ca`). Portée : gwine uniquement
  - `winewayland-keep-toplevel-for-client-surfaces.mypatch` — Dans `WAYLAND_WindowPosChanged` (`dlls/winewayland.drv/window.c`) : ne détache plus la client surface (surface de rendu D3D/Vulkan) quand la fenêtre perd sa wayland surface, et ne la ré-attache plus au toplevel — la client surface reste mappée en permanence. Évite des clignotements/pertes de rendu sous le driver Wayland. Origine : Soda 11.0-4 (bottlesdevs/wine commit `f295c6a`). Portée : gwine uniquement
...
- **Séparation cpuid/faketime** : `cpuid_fault_handler.mypatch` (signal_x86_64.c uniquement) est appliqué via le mécanisme standard de wine-tkg. `faketime.mypatch` est exclu des userpatches — le script `apply-faketime.py` est injecté dans `wine-tkg-scripts/prepare.sh` via `sed` avant `tools/make_requests`, ce qui permet à `make_requests` de régénérer les fichiers protocol à partir de `protocol.def` modifié. Cette séparation évite les conflits entre nos patches et les patches staging de wine-tkg. **La logique doit être identique dans `test-build.sh` ET `build-gwine.yml`** (le CI copie `apply-faketime.py` dans `/patches/` pour garder le même sed).
...
- Bundlé dans le package Wine :
  ...
  - **Deps runtime bundlées** (32-bit, `lib32/gst-libs/` + `lib/gst-libs/`) : `libdw.so.1` + `libelf.so.1` (linkés par libgstreamer-1.0 bundlé pour les backtraces GST_DEBUG) et `libOpenCL.so.1` (linké par `opencl.so` Wine 32-bit). **Pourquoi** : ces libs ne sont pas installées explicitement dans l'image hôte (Gablue/Bazzite) — elles arrivaient par des cascades de dépendances accidentelles, disparues le 20/09/2026 (retrait libheif du repo negativo17) → winegstreamer 32-bit mort au runtime (`quartz:autoplug Failed to create filter, hr 0x8007000e` sur tous les filtres, plus de vidéo dans les jeux 32-bit). `libzstd.so.1` 32-bit aussi (seul requérant système = elfutils-libelf.i686, lui-même sans requérant). `libz`/`liblzma` i686 restent système (requérants stables : fontconfig.i686, mesa multilib). Les fichiers sont sur le `LD_LIBRARY_PATH` du lanceur (`$WINE_DIR/lib32/gst-libs`) → trouvés sans rpath. Résolution du fix par analyse : `ldd` sur les plugins/libgs du runner + `nm -D` pour confirmer les symbols `dwfl_*` réellement appelés.
