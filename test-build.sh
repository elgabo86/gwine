...
# Hack upstream Valve "loader: HACK: Build wine64 and wine64-preloader" (commit 90855a2993e,
# activé sans condition) : preloader_exec() réécrit le loader 64-bit vers i386-unix/wine64[-
# preloader] (disposition du build Proton). gwine est compilé en wow64 séparé : les loaders
# 64-bit sont dans x86_64-unix/, et i386-unix/ ne contient que wine/wine-preloader 32-bit.
# Sans ces symlinks, Wine lance un loader 32-bit pour un process AMD64 → tentative de chargement
# de i386-windows/ntdll.dll → STATUS_IMAGE_MACHINE_TYPE_MISMATCH (0x4000000e) et wineboot échoue.
# Symlinks créés seulement si la cible existe et que i386-unix/ ne fournit pas déjà ces fichiers.
if [ -n "${UNIX32}" ] && [ -n "${UNIX64}" ]; then
  for f in wine64 wine64-preloader; do
    if [ -f "${UNIX64}/${f}" ] && [ ! -e "${UNIX32}/${f}" ]; then
      ln -sfn "$(realpath --relative-to="${UNIX32}" "${UNIX64}/${f}")" "${UNIX32}/${f}"
      echo "Symlink loader 64-bit: ${UNIX32}/${f} -> $(readlink "${UNIX32}/${f}")"
    fi
  done
fi
...
# Déps runtime elfutils/ocl-icd 32-bit : libgstreamer-1.0 (bundlé) link libdw.so.1
# (backtraces GST_DEBUG) et opencl.so (Wine 32-bit) link libOpenCL.so.1 — ces libs ne
# sont PAS installées explicitement côté image hôte (ocl-icd.i686 / elfutils-libs.i686
# venaient d'une cascade de dépendances disparue le 20/09/2026 → winegstreamer 32-bit
# mort : « Failed to create filter for GStreamer splitter filter, hr 0x8007000e »).
# Bundle dans gst-libs (sur le LD_LIBRARY_PATH du lanceur) pour un runner auto-portant.
# libzstd 32-bit embarqué aussi : son seul requérant système est elfutils-libelf.i686,
# lui-même sans requérant → présence fragile. libz/liblzma i686 restent système
# (requérants stables : fontconfig.i686 / mesa multilib explicites).
cp -a /usr/lib/libdw-*.so* /usr/lib/libdw.so.* "${GSTLIBS32_DIR}/" 2>/dev/null || true
cp -a /usr/lib/libelf-*.so* /usr/lib/libelf.so.* "${GSTLIBS32_DIR}/" 2>/dev/null || true
cp -a /usr/lib/libzstd.so.* "${GSTLIBS32_DIR}/" 2>/dev/null || true
cp -a /usr/lib/libOpenCL.so.1* "${GSTLIBS32_DIR}/" 2>/dev/null || true
cp -a /usr/lib/libdw-*.so* /usr/lib/libdw.so.* "${GSTLIBS_LIB_DIR}/" 2>/dev/null || true
cp -a /usr/lib/libelf-*.so* /usr/lib/libelf.so.* "${GSTLIBS_LIB_DIR}/" 2>/dev/null || true
cp -a /usr/lib/libzstd.so.* "${GSTLIBS_LIB_DIR}/" 2>/dev/null || true
cp -a /usr/lib/libOpenCL.so.1* "${GSTLIBS_LIB_DIR}/" 2>/dev/null || true
